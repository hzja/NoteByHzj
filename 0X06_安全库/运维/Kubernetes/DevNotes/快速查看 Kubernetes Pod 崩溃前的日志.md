Kubernetes<br />当pod处于crash状态的时候，容器不断重启，此时用 **kubelet logs** 可能出现一直捕捉不到日志。解决方法：<br />**kubectl previous** 参数作用：**If true, print the logs for the previous instance of the container in a pod if it exists.**
