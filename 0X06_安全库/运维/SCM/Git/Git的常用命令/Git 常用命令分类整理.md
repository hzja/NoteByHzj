...
<a name="wGBzt"></a>
### 添加
添加文件到当前工作空间中。如果不使用 `git add` 将文件添加进去，那么这些文件也不会添加到之后的提交之中
```bash
# 添加一个文件
$ git add test.js
...
<a name="h4jHF"></a>
### rebase (谨慎使用)
将一个分支上所有的提交历史都应用到另一个分支上<br />不要在一个已经公开的远端分支上使用 rebase.
```bash
...
<a name="XSxx0"></a>
### reset (谨慎使用)
将当前的头指针复位到一个特定的状态。这样可以撤销 `merge`、`pull`、`commits`、`add` 等<br />这是个很强大的命令，但是在使用时一定要清楚其所产生的后果
```bash
