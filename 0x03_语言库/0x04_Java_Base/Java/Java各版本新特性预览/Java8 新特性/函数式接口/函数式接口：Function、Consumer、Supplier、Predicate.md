...
<a name="G2CuH"></a>
## 前言
Java 8 中提供了许多函数式接口，包括Function、Consumer、Supplier、Predicate 等等。这 4 个接口就是本篇将要分享的内容，它们都位于 java.util.function 包下。<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/396745/1680708992479-62d45503-c06e-4f78-940b-afac5f921808.png#averageHue=%23fafcde&clientId=ua6ab8fb9-7357-4&from=paste&height=702&id=u439329e7&originHeight=1755&originWidth=835&originalType=binary&ratio=2.5&rotation=0&showTitle=false&size=156516&status=done&style=none&taskId=u92e1c657-bf7a-4bbb-b514-88e0e0bd64e&title=&width=334)
<a name="U8XXi"></a>
## 为什么需要知道这几个函数式接口？
因为这 4 个函数式接口是 Java 8 中新增的重要接口，同时 Java 8 的 Stream 新特性，也有用到这些接口，所以学习它们可以更好地理解 Stream 流。<br />也正因为这是函数式接口，所以就可以使用 Lambda 表达式来写接口的实现逻辑。而且学习的过程中可以更好地理解函数式编程的思想。
<a name="hMacj"></a>
## Function 接口
<a name="CeWsj"></a>
### 说明
Function 这个单词的意思就有「函数」的意思，就数学中的 y = f(x)，接收一个 x 参数，通过函数 f 运算后，返回一个结果 y。<br />Function 接口包含四个方法：
...
<a name="NE2gy"></a>
### 说明
Consumer 这个单词的意思就有「消费者」的意思，就把入参消费了，并不会返回结果给你。<br />Consumer 接口包含两个方法：
...
<a name="xDfUr"></a>
## Supplier 接口
Supplier 接口只定义了一个 `get()` 方法，该方法不接受任何参数并返回一个结果。<br />Supplier 这个单词的意思就有「供应者」的意思，给我的感觉就是生产者，不用参数，直接生产一个东西给你。<br />Supplier 接口通常用于生成一个值。例如：
```java
...
<a name="VCChd"></a>
### 说明
Predicate 这个单词的意思就有「预言，预测，谓语，谓词」的意思，就是用来预测判断的。<br />Predicate 接口包含四个方法：
...
<a name="sJPrV"></a>
## 那这些接口有什么应用呢？
在 Stream 流中就有应用上这些函数式接口。当然，当有相似的需求时，也可以应用上这些接口。下面说下 Stream 流中的应用。<br />Function 接口：例如 map 方法，map 方法就是将一个类型的值转换为另一个类型的值。
```java
