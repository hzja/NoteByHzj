...
<properties>
  ...
  <flying-saucer-pdf.version>9.1.1</flying-saucer-pdf.version>
</properties>
```
<a name="1nAo7"></a>
### UploadUtils.java
...
public class TestController {
    ...
    public void identifyPicture(HttpServletResponse response) {
        //1.在内存中创建一张图片
        BufferedImage bi = new BufferedImage(UploadUtils.WIDTH, UploadUtils.HEIGHT, BufferedImage.TYPE_INT_RGB);
        //2.得到图片
        Graphics g = bi.getGraphics();
        //3.设置图片的背影色
        UploadUtils.setBackGround(g);
        //4.设置图片的边框
        UploadUtils.setBorder(g);
        //5.在图片上画干扰线
        ...
        String random = UploadUtils.drawRandomNum((Graphics2D) g, "nl");
