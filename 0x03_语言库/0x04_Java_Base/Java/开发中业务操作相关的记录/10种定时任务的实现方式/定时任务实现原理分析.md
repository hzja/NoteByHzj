Java 定时任务<br />在很多业务的系统中，常常需要定时的执行一些任务，例如定时发短信、定时变更数据、定时发起促销活动等等。<br />定时任务主要分**单机**和**分布式**两大类。从单机角度，定时任务实现主要有以下 3 种方案：
...
- 回到最开始说到的`ScheduledFutureTask`任务线程类，最终执行任务的其实就是它
