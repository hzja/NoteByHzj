...
<a name="Oofrr"></a>
### 9.2. 类型擦除后保留的原始类型
1、在上面，几次提到了**原始类型**。什么是原始类型?原始类型就是擦除去了泛型信息，最后在字节码中的类型变量的真正类型。无论何时定义一个泛型类型，相应的原始类型都会被自动地提供。类型变量被擦除，并使用其限定类型（无限定的变量用Object替换）。<br />例3：
```java
